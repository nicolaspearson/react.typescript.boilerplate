...
const DEFAULT_HEAD = [<meta charSet="utf-8" className="react-ts-head" />];
...
export default class Head extends React.Component {
	...
	constructor(props: HeadProps, context?: any) {
		super(props, context);
	...
	public updateElements(type: any, components: any) {
		...
		const oldTags = Array.prototype.slice.call(headEl.querySelectorAll(type + '.react-ts-head'));
	...
	public render() {
		const head: Array<React.DetailedReactHTMLElement<any, any>> = React.Children.map(
			...
			.reverse()
			.map((c: any) => {
				const className = (c.className ? c.className + ' ' : '') + 'react-ts-head';
